...
export default function AdminLayout() {
  ...
  return (
    <div data-testid="admin-layout" className="relative flex min-h-screen bg-slate-50 overflow-x-hidden md:h-screen md:overflow-hidden">
      {isMobile ? (
        <>
          {isMobileSidebarOpen && (
            <button
          ...
          >
            <Sidebar data-testid="admin-nav" currentPage={currentPage} onNavigate={handleNavigate} />
          </div>
        </>
      ) : (
        <Sidebar data-testid="admin-nav" currentPage={currentPage} onNavigate={handleNavigate} />
      ...
      <div className="min-w-0 flex-1 overflow-x-auto overflow-y-auto flex flex-col">
        <Header data-testid="admin-header" onToggleSidebar={() => setIsMobileSidebarOpen(true)} />
